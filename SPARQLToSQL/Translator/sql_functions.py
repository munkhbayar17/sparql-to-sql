                                ...
                                if expr.valueLogical()[0].relationalExpression().numericExpression():
                                    ...
                                    if expr.additiveExpression():
                                        if expr.additiveExpression().multiplicativeExpression():
                                            if expr.additiveExpression().multiplicativeExpression()[
                                                0].unaryExpression():
                                                expr = expr.additiveExpression().multiplicativeExpression()[
                                                    0].unaryExpression()[0]
                                                if expr.primaryExpression():
                                                    if expr.primaryExpression().var():
                                                        order_list.append(SQLOrderField(
                                                            get_var(expr.primaryExpression().var()).context,
                                                            getTerminalNode(order_condition)))
                                                        return order_list
