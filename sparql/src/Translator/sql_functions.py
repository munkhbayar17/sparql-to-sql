from sparql.src.Helpers.sql_helper import *
from sparql.src.Helpers.translation_helper import *
from sparql.src.Helpers.parser_helper import get_var, getTerminalNode
from sparql.src.Translator.mapping import *
...
def join_SQL_objects(sql_obj1, sql_obj2, join_type, trans_type, is_union=False):
	if trans_type == 3:
		if is_union == False:
			if join_type == SQL_INNER_JOIN:
				if isinstance(sql_obj1, SQLQuery) & isinstance(sql_obj2, SQLQuery):
					return ext_join_SQL_objects(sql_obj1, sql_obj2)
	...
	if is_union:
		...
		pr_list = build_join_pr(sql_obj1, sql_obj2, join_type, trans_type)

	# conditions
	conditions = None

	if is_union == False:
		conditions = build_join_conditions(sql_obj1, sql_obj2, trans_type)
...
def build_join_pr(sql_obj1, sql_obj2, join_type, trans_type):
	pr_list = []
	...
	for common_pr in common_fields:
		...
		if (trans_type == 1 or join_type_of_sql_obj(sql_obj1, SQL_LEFT_OUTER_JOIN)):
...
def build_join_conditions(sql_obj1, sql_obj2, trans_type):
	condList = []
	...
	for project in common_fields:
		...
		condList.append(SQLJoinCondition(project1, OP_EQUALS, project2, (trans_type != 1)))
...
def unite_sql_objects(sql_obj1, sql_obj2, trans_type):
	if trans_type == 1:
		union_part1 = join_SQL_objects(sql_obj1, sql_obj2, SQL_LEFT_OUTER_JOIN, 1, True)
		union_part2 = join_SQL_objects(sql_obj2, sql_obj1, SQL_LEFT_OUTER_JOIN, 1, True)
	else:
...
# Extension functions
def ext_join_SQL_objects(sql_query_obj_1, sql_query_obj_2):
	"""
